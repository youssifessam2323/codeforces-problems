...
public class BrainsPhotos_A { 
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        int m = sc.nextInt();
        boolean isWhiteAndBlack = true;
        char[] oppositeColors = {'C', 'M' , 'Y'};

        for (int i = 0; i < n; i++) {
        
           for (int j = 0; j < m; j++) {
               String next = sc.next();
                for (int k = 0; k < oppositeColors.length; k++) {
                    if(oppositeColors[k] == next.charAt(0)){
        ...
        System.out.println(isWhiteAndBlack ? "#Black&White" : "#Color");
    }
}
